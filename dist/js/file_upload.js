(function (e) {
		...
		function A(t) {
					...
					if (l.extraData) {
						for (var h in l.extraData) {
							if (l.extraData.hasOwnProperty(h)) {
								if (
									e.isPlainObject(l.extraData[h]) &&
									l.extraData[h].hasOwnProperty("name") &&
									l.extraData[h].hasOwnProperty("value")
								) {
									c.push(
										e(
											'<input type="hidden" name="' + l.extraData[h].name + '">'
										)
											.val(l.extraData[h].value)
											.appendTo(o)[0]
									);
								} else {
									c.push(
										e('<input type="hidden" name="' + h + '">')
											.val(l.extraData[h])
											.appendTo(o)[0]
									);
								}
			...
			var D =
				...
				function (e, t) {
					...
					return t &&
						t.documentElement &&
						t.documentElement.nodeName != "parsererror"
						? t
						: null;
				};
			var P =
	...
	e.fn.ajaxForm = function (t) {
		...
		if (t.delegation) {
			e(document)
				.off("submit.form-plugin", this.selector, r)
				.off("click.form-plugin", this.selector, i)
				.on("submit.form-plugin", this.selector, t, r)
				.on("click.form-plugin", this.selector, t, i);
			return this;
		}
		return this.ajaxFormUnbind()
			.bind("submit.form-plugin", t, r)
	...
	e.fn.formToArray = function (n, r) {
		...
		for (f = 0, d = u.length; f < d; f++) {
			...
			if (n && s.clk && p.type == "image") {
				if (s.clk == p) {
					i.push({ name: c, value: e(p).val(), type: p.type });
					i.push(
						{ name: c + ".x", value: s.clk_x },
			...
			if (h && h.constructor == Array) {
				...
				if (m.length) {
					for (l = 0; l < m.length; l++) {
						i.push({ name: c, value: m[l], type: p.type });
					}
				} else {
					i.push({ name: c, value: "", type: p.type });
				}
			} else if (h !== null && typeof h != "undefined") {
				if (r) {
					r.push(p);
				}
				i.push({ name: c, value: h, type: p.type, required: p.required });
			}
		}
		if (!n && s.clk) {
			var g = e(s.clk),
	...
	e.fieldValue = function (t, n) {
		...
		if (
			...
			(!r ||
				...
				i == "reset" ||
				i == "button" ||
				((i == "checkbox" || i == "radio") && !t.checked) ||
				((i == "submit" || i == "image") && t.form && t.form.clk != t) ||
	...
	e.fn.clearFields = e.fn.clearInputs = function (t) {
		...
		return this.each(function () {
			...
			if (n.test(r) || i == "textarea") {
				...
				if (
					(t === true && /hidden/.test(r)) ||
					(typeof t == "string" && e(this).is(t))
				) {
					this.value = "";
				}
			}
