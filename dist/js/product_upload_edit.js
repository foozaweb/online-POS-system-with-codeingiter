...
$("#submitProductBtn").on("click", function () {
	...
	var productColor = $('[name="color"]').val();
	var productImages = $(".productImages").html();
	var main_photo = $("[name='main_photo']").val();
	...
	$(".validate").each(function () {
		...
		if (!val) {
			...
			toastr.danger(capitalizeFirstLetter(msg) + " is empty");
	...
	if (!productColor) {
		toastr.danger("Please select available colors");
		return false;
	}

	if (!productDesc) {
		toastr.danger("Please Describe this product");
		return false;
	}

	if (!main_photo) {
		toastr.danger("Click any of the images uploaded to set front photo");
		return false;
	}

	if (!productImages) {
		toastr.danger("Please Upload images for this product");
		return false;
	}

	if (main_photo && productImages && productColor) {
		var totalCount = $(".myField").length;
		...
		if (err) {
			...
			toastr.info("Product Successfully Updated. Refreshing page shortly");
			thereIsAnActivity(
				"Updated Product: " +
...
$("#files2").on("change", function () {
	var fi = document.getElementById("files2");
	if (!fi) {
		return false;
	}
		...
		function readImageFile(file) {
			...
			reader.onload = function (e) {
				...
				img.onload = function () {
					...
					if (size > 2000) {
						$("#preview").html(
							"<div class='red'><strong>Please select images with the following parameters:</strong> <br> Max Size: 2mb</div>"
						);
						$("#files2").val("");
						return false;
					} else {
...
function beginUpload2() {
	var files = $("#files2")[0].files;
	if (files.length < 1) {
		alert("Error! Please Provide image data for upload");
		return false;
	}
	var error = "";
	...
	if (error == "") {
		$.ajax({
			url: base_url + "app/upload_product_pics",
			method: "POST",
			data: form_data,
			contentType: false,
			cache: false,
			...
			success: function (data) {
				if (confirm("Do you want to replace existing images?")) {
					$("#preview").html(data);
				} else {
					$("#preview").append(data);
				}
				...
				$(".uploaded_img").each(function () {
					if ($(".productImages").html() == "") {
						$(".productImages").html($(this).data("image_name"));
					} else {
						$(".productImages").append(", " + $(this).data("image_name"));
					}
				});
...
var imgHtml = "";
var imgName = "";
$("#preview").on("dblclick", ".uploaded_img", function () {
	var imgHtml = $(".productImages").html();
	var imgName = $(this).data("image_name");
	if (imgHtml.indexOf(imgName) > -1) {
		imgHtml.replace(imgName, "");
		console.log(imgHtml);
	}

	$(".productImages").html(imgHtml);
	$("#" + $(this).data("id")).remove();
	$('[name="main_photo"]').val("");
	toastr.info("Photo Successfully Removed");
});
...
$("#preview").on("click", ".uploaded_img", function () {
	$('[name="main_photo"]').val($(this).data("image_name"));
});
...
function getSubCategory(cat) {
	$.ajax({
		...
		success: function (data) {
			...
			$("#subCatSelect").html(
				"<option disabled selected value=''>Select Sub Category</option>" + html
			);
		},
		error: function (data) {
